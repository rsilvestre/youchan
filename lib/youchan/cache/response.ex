defprotocol Youchan.Cache.Response do
  @moduledoc """
...
defimpl Youchan.Cache.Response, for: Tuple do
...
defimpl Youchan.Cache.Response, for: Atom do
  def format(nil), do: {:miss, nil}
  def format(:ok), do: {:ok, :ok}
  def format(atom), do: {:ok, atom}
end

defimpl Youchan.Cache.Response, for: Any do
  def format(nil), do: {:miss, nil}
