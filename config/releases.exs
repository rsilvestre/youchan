...
if youtube_api_key = System.get_env("YOUTUBE_API_KEY") do
  config :youchan, youtube_api_key: youtube_api_key
else
  ...
  cache_backends: %{
    channel_details: %{
      ...
      backend_options: [
        ...
        cache_dir: "/app/priv/youchan_cache",
        max_size: 10000
      ]
    }
  }
...
if System.get_env("USE_DISTRIBUTED_CACHE") == "true" do
  config :youchan,
    cache_backends: %{
      channel_details: %{
        backend: Youchan.Cache.CachexBackend,
        backend_options: [
          table_name: :channel_details_cache,
          distributed: true
        ]
...
if System.get_env("API_REQUEST_TIMEOUT") do
  config :youchan, api_request_timeout: String.to_integer(System.get_env("API_REQUEST_TIMEOUT"))
end
